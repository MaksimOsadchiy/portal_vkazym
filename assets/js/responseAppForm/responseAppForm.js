document.addEventListener('DOMContentLoaded', async () => {
	...
	const getData = async () => {
		try {
			...
			const response = await fetch(`${NEW_SERVER_URL}allApplications`);
	...
	const postAppResponse = async () => {
		try {
			...
			// Формируем запрос к серверу
			const response = await fetch(`${NEW_SERVER_URL}response`, {
				method: 'POST',
				headers: {
	...
	const addEventRow = (elem) => {
		elem.addEventListener('click', async () => {
			// Вешаем слушатель на событие 'click'
			...
			modalBodyStatus
				...
				.forEach((elem) => (+elem.value <= initialStatus || +initialStatus === 2 ? (elem.disabled = true) : (elem.disabled = false)));
		});
	};
	/**
