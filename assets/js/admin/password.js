...
const updatePassword = async (id, params, table) => {
	try {
		...
		if (!response.ok) {
			throw new Error(jsonResponse.status || 'Ошибка обновления данных');
		}
		document.dispatchEvent(
			new CustomEvent('updateError', { detail: 'Пароль сохранен!' })
		);
		return true;
	} catch (error) {
...
const addEventResetPassBtn = () => {
	document
		.querySelector('.btn-reset-password')
